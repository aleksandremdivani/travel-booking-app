...
const BookingPage = () => {
  ...
  const insertBooking = async () => {
    const results = await Promise.all(
      selectedRooms.flatMap((hotel) => {
        return hotel.selectedRooms.map((room) => {
          const nights = differenceInDays(
            new Date(room.check_out),
            new Date(room.check_in),
          );
          ...
          const { id, name: hotelName, main_photo, city, country } = hotel;

          return supabase.from("bookings").insert({
            user_id: user.id,
            hotel_id: id,
            hotel_name: hotelName,
            hotel_image: main_photo,
            price: nights * price,
    ...
    console.log(results);
    const hasErrors = results.some((i) => i.error);
    if (!hasErrors) {
      setIsBooked(true);
      setSelectedRooms([]);
    }
  ...
  const totalPrice = hotelFullPrice.reduce(
    (sum, item) => item.eachHotelPrice + sum,
    0,
  );
    ...
    <main className="bg-[#FCFCFC] text-[#111111] border-t">
      <div className="max-w-[1280px] px-10 pt-10 pb-20 flex flex-col items-center lg:flex-row gap-8 items-start">
            ...
            >
              ...
              <div>
                ...
                {hotel?.selectedRooms?.map((item) => {
                  ...
                  const price = Math.round(
                    item.rates?.[0]?.retailRate?.total?.[0]?.amount,
                  );
                  ...
                  return (
                    ...
                    >
                      ...
                      <div className="flex h-full min-h-[120px] flex-col items-end justify-between pl-8">
                        ...
                        <button
                          onClick={() => handleRoomSelection(item, hotel)}
        ...
        <aside className="w-80 lg:sticky lg:top-8 rounded-2xl border border-[#E7E7E7] bg-[#FAFAFA] p-6">
          ...
          <h2 className="mt-3 text-[24px] leading-tight font-medium text-[#111111]">
            Booking total
          </h2>
          ...
          <div className="mt-8 space-y-4">
            {hotelFullPrice.map((hotel) => (
              ...
              >
                ...
                <span className="text-[15px] text-[#111111] whitespace-nowrap">
                  ${hotel.eachHotelPrice}
                </span>
              </div>
            ))}
          </div>
          ...
          <div className="flex items-center justify-between">
            ...
            <span className="text-[28px] font-medium tracking-[-0.03em] text-[#111111]">
              ${totalPrice}
            </span>
          </div>
          ...
          <button
            ...
            className="hover:bg-black active:scale-[0.95] hover:backdrop-blur-sm mt-8 h-14 w-full rounded-xl bg-[#111111] text-[15px] font-medium text-white transition-colors bg-black/80"
          >
            Confirm booking
          </button>
        </aside>
